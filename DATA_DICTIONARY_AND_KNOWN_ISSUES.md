...
## 02_Northbridge_Software_SaaS/ (QuickBooks Online + Stripe)

| File | What it is |
|---|---|
| `GL_Export_QBO_Jan-Dec2025.csv` | Full general ledger export — revenue, COGS, payroll, AP bills, commissions, depreciation, intercompany interest, and manual JEs, 672 lines |
...
| File | What it is |
|---|---|
| `GL_Export_Xero_Jan-Dec2025.csv` | Full GL export — sales by channel, COGS, payroll, AP bills, depreciation, intercompany, manual JEs, 570 lines. Uses a completely different account-numbering scheme than the SaaS entity. |
...
approval matrix, vendor master, standardized chart of accounts, intercompany
policy). `Scattered_Notes_and_Tribal_Knowledge.md` collects a handful of
informal emails/Slack messages that function as the only "policy" that
...
- **No data warehouse.** Nothing here has been normalized into a common
  schema. Every file uses that source system's own field names, date formats,
