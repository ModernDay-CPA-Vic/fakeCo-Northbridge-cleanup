...
- **Finance team:** One fractional Controller (3 days/week, spread across all four
  ...
  no FP&A function. No dedicated system tying the entities together.
...
| **Northbridge Software, Inc.** | B2B SaaS — "BuildFlow" project management software for construction & trades companies | NetSuite (single-subsidiary instance) + Stripe | ~$5.1M | 45 |
...
Northbridge Software runs on NetSuite, licensed with OneWorld (multi-subsidiary
consolidation) as part of the package — but only Northbridge Software was ever
actually implemented on it. Verve Supply Co., Northbridge Advisory, and Corporate
were supposed to migrate onto the same instance "eventually," and never did. So
the one piece of real ERP infrastructure this company owns is sitting there
underused: it could consolidate all four entities today, and instead it's running
as a glorified single-entity general ledger while Priya still rebuilds the group
trial balance by hand in Excel every month. Extending the existing NetSuite
OneWorld setup to the other three entities is one of the more obvious "quick win"
recommendations this project should surface.
...
integrated SaaS company on NetSuite, one is a scrappy DTC brand on Xero, one is a
services firm that never adopted accounting software at all — and nobody has ever
unified them. The result, deliberately reproduced in this dataset:

1. **No data warehouse or consolidation tool in active use.** Four entities, four
   unconnected systems (NetSuite, Xero, Excel, Excel) feed one manually-rebuilt
   Excel workbook every month-end, even though NetSuite OneWorld could technically
   do this today if the other three entities were onboarded.
...
4. **Reconciliation gaps across the board.** AR subledgers, deferred revenue,
   inventory, and intercompany balances that don't tie to the GL.
5. **Inconsistent use of the segments NetSuite already offers.** Even within
   NetSuite, the Department segment is tagged on some transactions and not
   others, with no posting policy enforcing it.
6. **No documented policies.** See `05_Policies_and_Controls/` — there isn't a
