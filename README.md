...
|---|---|---|---|
| Northbridge Software, Inc. | B2B SaaS (construction/trades project management) | NetSuite (single-subsidiary instance) + Stripe | ~$5.1M |
...
Northbridge Software is the one entity on a real ERP — NetSuite, licensed
with OneWorld multi-subsidiary consolidation as part of the package. But
OneWorld was never turned on for the other three entities, so today it runs
as a glorified single-entity general ledger while the Controller rebuilds
the group trial balance by hand in Excel every month. Extending OneWorld to
the rest of the group — instead of buying or building a separate
consolidation tool — is one of the clearer "quick win" calls this project
surfaces in Section 3.
